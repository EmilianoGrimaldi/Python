...
def obtener_oraciones(cadena:str)->list:
    """Filtra oraciones con delimitadores

    Args:
        cadena (str): La cadena a evaluar

    Returns:
        list: La lista filtrada con las oraciones separadas por los delimitadores
    """
    patron = compile(f"[\s¿][\w\s,*¡]+[\s\W]")
    lista_filtrada = findall(patron, cadena)
